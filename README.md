...
**Recording** — the [`Replay`](GameFiles/Gameplay%20Scripts/Replay.gd) autoload starts on the first touch (`gameStarter` calls `Replay.start_run()`), samples the player's position/rotation at 20 Hz in `_physics_process` (so pauses don't record), and logs every drawn line segment as `playerInput` lays it. On game over the finished recording is **auto-saved as the rolling last replay** (`last.srp` in `user://replays/`, overwritten every run — always the first entry in the browser). To keep a run permanently, **rename it** in the Settings replay browser: once it has another name, the next run's auto-save can't touch it (the same model as BombSquad's replays). Quitting or restarting mid-run discards the recording.
...
**Watching** — the Settings panel lists the saved replays (only files the current format can play — stale/incompatible ones are hidden) with a **rename field** (how you keep a run — see above) and an **OPEN REPLAYS FOLDER** button for backing up or sharing the files; picking one opens [`ReplayViewer.tscn`](GameFiles/Sprites/UI/ReplayViewer.tscn) ([`ReplayViewer.gd`](GameFiles/Gameplay%20Scripts/ReplayViewer.gd)). Self-animated obstacles keep their scripts and simply run (pendulums swing, dragons slither, bats and falling lights react to the puppet player, which joins the "Player" group for exactly that reason); missiles and the clone are stripped to visuals and driven along their recorded paths, exploding at their recorded deaths. The viewer's World node runs the **real [`changeSize.gd`](GameFiles/ScreenView%20Scripts/changeSize.gd) playfield script** — the same aspect-ratio handling as every mode and the menu — and the puppet keeps the genuine player Camera2D (per-mode framing, y-lock, look-ahead smoothing, CameraShake), so replays are framed exactly like gameplay on any screen. The World also hosts the **real `indicatorManager`**, so obstacle warnings — airships, hotel, hanging bars, dragons — spawn, beep and track just like in the live run; stand-in `PowerupPopUps` nodes driven by the recorded HUD state steer reconstructed buildings' powerup rolls. Presentation matches gameplay: a random gameplay track plays, star/powerup/explosion sounds fire on their events, missile warning markers beep (with Missiles-mode's tracking arrows), missiles detonate at their recorded moment, the carried jetpack burns its flames and thrust hum, the speed trail streams behind the puppet, and drawn lines keep their speed-based thickness and brighten when the ball rolls over them. A **timeline scrubber** with a **pause/play button** seeks anywhere in the run — rewinding wipes the reconstructed world and replays the recording's state up to the target moment in one pass, and pausing freezes the whole replay (world, sounds and music) while the timeline stays scrubbable.
